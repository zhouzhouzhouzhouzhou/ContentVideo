package com.example.content.CustomView;
...
import com.example.content.Util.DensityUtils;
...
public class CustomMediaController extends MediaController {
    ...
    private RelativeLayout ll_custom_controller;
...
//                activity.finish();
            ...
            if (activity != null) {
                switch (activity.getResources().getConfiguration().orientation) {
                    ...
                    case Configuration.ORIENTATION_PORTRAIT://竖屏
                        ...
                        videoView.setVideoLayout(VIDEO_LAYOUT_SCALE,0);
                        activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
                        break;
                }
            }
    ...
    //framelayout 是为了将播放控制条设置到视频上面
    //public CustomMediaController(Context context, VideoView videoView, Activity activity, FrameLayout anchorView) {
...
//        videoView.getVideoHeight()
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (lp==null){
             ...
             lp = (RelativeLayout.LayoutParams) ll_custom_controller.getLayoutParams();
        }
        if (w>h){
            //全屏
//            videoView.setVideoLayout(VIDEO_LAYOUT_SCALE,0);
            lp = (RelativeLayout.LayoutParams) ll_custom_controller.getLayoutParams();
            lp.height = 1280;
            Log.i(TAG, "全屏onSizeChanged: "+lp.height);
        }else{
            //竖屏
            lp = (RelativeLayout.LayoutParams) ll_custom_controller.getLayoutParams();
            lp.height= (int) DensityUtils.dp2px(context,221);
            Log.i(TAG, "竖屏onSizeChanged: "+lp.height);
    ...
    protected View makeControllerView() {
        ...
        View v = ((LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(getResources().getIdentifier("custom_mediacontroller", "layout", getContext().getPackageName()), this);
        ...
        ll_custom_controller = (RelativeLayout) findViewById(R.id.ll_custom_controller);
//        anchorView = (FrameLayout) findViewById(R.id.anchorview);
        return v;
    }
